...
public class Sender extends CustomComponent {
	...
	private void setVisible(Boolean visible){
		dane.getSenderDane().getTextArea().setVisible(visible);
		
		//dane.getSenderDane().getTable().setVisible(visible);
		//dane.getSenderDane().getButton().setVisible(visible);
		combobox.setVisible(visible);
		buttonToaddType.setVisible(visible);
	...
	private void createTextArea() {
       ...
       // ustawienie widoczności
		dane.getSenderDane().getTextArea().setVisible(false);
	...
	/*private void createTextLoginAs() {
		dane.getSenderDane().getMainLayout()
		.addComponent(loginAs, "top:650.0px;left:10.0px;");
	...
	private void createLoginArea() {
		dane.getSenderDane().getMainLayout()
				.addComponent(loginField(), "top:400.0px;left:10.0px;");
	}

	private void createPasswordArea() {
		dane.getSenderDane().getMainLayout()
		.addComponent(passwordField(), "top:500.0px;left:10.0px;");
	}
	
	/*private void createLoginButton() {
		final BeanFieldGroup<Person> binder = new BeanFieldGroup<Person>(
				Person.class);
		login(container,binder);
		loginButton.setImmediate(true);
		
		loginField.setValue("");
		loginField.discard();
		dane.getSenderDane().getMainLayout()
		.addComponent(loginButton(), "top:550.0px;left:40.0px;");
	}
*/
	private void createSendButton() {
		...
		dane.getSenderDane().getButton().setVisible(false);
		dane.getSenderDane()
				.getMainLayout()
	...
	private void createButtons() {
		...
		loginButton= new Button("login", new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				System.out.println(loginField.getValue());
				final BeanFieldGroup<Person> binder = new BeanFieldGroup<Person>(
						Person.class);
				try {
					login(container,binder);
				...
				new Button("Send", new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						if (addresField.isValid()) {
							...
							getWindow().showNotification(
									"Pleas imput correct addres area!!!");
							addresField.discard();
	...
	private void login(final JPAContainer<Type> container,
			final BeanFieldGroup<Person> binder) throws Exception {
				
				String login=loginField.getValue().toString();
	...
	private void loginAs(String login,String password)throws Exception {
		...
		try{
		Person p=dane.getSenderDane().getUser(login,password);
		
		dane.getSenderDane().getButton().setVisible(true);
		dane.getSenderDane().getTextArea().setVisible(true);
	...
	private void getMessageLoginAs(Person p) {
		loginAs= new Label();
		loginAs.setStyleName("label");
	     loginAs.setValue("Zalogowany jako: "+p.getUser());
	     dane.getSenderDane().getMainLayout()
			.addComponent(loginAs, "top:10.0px;left:300.0px;");
	}
	/*
	 * private void sendButton(final JPAContainer<Type> container, final
	 * BeanFieldGroup<Message> binder) { dane.getSenderDane().setButton( new
	 * Button("Send", new Button.ClickListener() {
	 * 
	 * @Override public void buttonClick(ClickEvent event) {
	 * 
	 * login(container, binder); container.refresh(); refreshtable();
	 * 
	 * 
	 * }
	 * 
	...
	/*private Component loginAsField() {
		
		loginAs= new TextField("Zalogowany Jako:");
		return loginAs;
	}*/
	...
	public void remove(Component component) {
		dane.getSenderDane().getMainLayout().removeComponent(component);
